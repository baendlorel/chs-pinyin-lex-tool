...
function createEntryId(rawHeader, pinyin, index, text) {
    return crypto.createHash('sha1').update(rawHeader).update('\0').update(pinyin).update('\0').update(String(index)).update('\0').update(text).digest('hex');
}
function readUtf16z(buffer, start, end) {
    let pointer = start;
    while (pointer + 1 < end) {
...
function parseRecord(buffer, start, end) {
    if (end - start < 16) {
        return null;
    }
...
function parseLexBuffer(filePath, buffer) {
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        const end = index + 1 < count ? recordStart + offsets[index + 1] : Math.min(totalSize || buffer.length, buffer.length);
...
function buildRecord(entry, templateHeader) {
...
function getTemplateHeader(entry) {
    return entry.rawHeaderBase64 ? Buffer.from(entry.rawHeaderBase64, 'base64') : undefined;
}
function buildLexBuffer(entries, oldLex) {
    const sortedEntries = entries
        .map((entry) => ({ ...entry }))
        .sort((left, right) => {
        return (left.pinyin.localeCompare(right.pinyin, 'zh-Hans-CN') ||
            left.index - right.index ||
            left.text.localeCompare(right.text, 'zh-Hans-CN'));
    });
    const records = sortedEntries.map((entry) => buildRecord(entry, getTemplateHeader(entry) ?? oldLex?.entries[0]?.rawHeader));
...
function toSerializedEntry(record) {
    return {
...
function getBackupPath(filePath, backupIndex) {
    return `${filePath}.bak${backupIndex}`;
}
export function listLexBackups(filePath) {
...
export function rotateLexBackups(filePath) {
...
export function sanitizeLexEntries(entries) {
...
export function scanLexDirectory(inputPath) {
    ...
    const selectedFileName = stats.isFile() && resolvedPath.toLowerCase().endsWith('.lex') ? path.basename(resolvedPath) : null;
    const files = fs
...
export function loadLexFile(filePath) {
...
export function saveLexFile(filePath, entries) {
...
export function restoreLexBackup(filePath, backupIndex) {
