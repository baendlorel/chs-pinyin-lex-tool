...
import { loadLexFile, normalizeWindowsPath, parseImportText, restoreLexBackup, saveLexFile, scanLexDirectory, } from './lex-service.js';
...
function resolveLexFilePath(payload) {
...
function createServer() {
    ...
    app.post('/api/lex/import', async (request) => {
        const payload = request.body;
    ...
    app.post('/api/lex/restore', async (request) => {
        const payload = request.body;
    ...
    app.get('/api/path/normalize', async (request) => {
        const source = assertNonEmptyString(request.query?.input, 'input');
        return {
            input: source,
